    ...
    <div>
        ...
        <form action="{{ route('admin.jadwalmapel.index') }}" method="GET" class="filter-scroll mb-3">
            ...
            <select name="hari" class="form-control" style="width:120px;">
                ...
                @foreach (['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat'] as $h)
                    <option value="{{ $h }}" {{ request('hari') == $h ? 'selected' : '' }}>{{ $h }}
                    </option>
                @endforeach
            </select>
            ...
            <select name="id_mapel" class="form-control" style="width:160px;">
                ...
                @foreach ($mapelList as $mapel)
                    <option value="{{ $mapel->id_mapel }}"
                        {{ request('id_mapel') == $mapel->id_mapel ? 'selected' : '' }}>
                        {{ $mapel->nama_mapel }}
                    </option>
                @endforeach
            </select>
            ...
            <input type="text" name="tahun_ajaran" placeholder="Thn Ajaran" class="form-control" style="width:110px;"
        ...
        @if (session('success'))
            <div id="flash-message"
                style="background:#d4edda; border:1px solid #c3e6cb; color:#155724; padding:12px 16px; border-radius:6px; 
                    margin-top:20px; margin-bottom:20px; transition: opacity 0.5s ease;">
                {{ session('success') }}
            </div>
                ...
                <tbody>
                    @foreach ($jadwalList as $jadwal)
                        <tr>
                            ...
                            <td>{{ $jadwal->kelas?->tingkat }} {{ $jadwal->kelas?->jurusan }}
                            ...
                            <td data-label="Aksi">
                                <div class="aksi-container">
                                    <a href="{{ route('admin.jadwalmapel.edit', $jadwal->id_jadwal) }}"
                                        class="btn-action btn-green"><i class="bi bi-pencil-square"></i></a>
                                    <a href="{{ route('admin.jadwalmapel.show', $jadwal->id_jadwal) }}"
                                        class="btn-action btn-green"><i class="bi bi-eye"></i></a>
